...
class DDHNet(nn.Module):
    def __init__(self, n_classes, n_channels, pretrained_model=None, norm_layer=nn.BatchNorm2d):
        ...
        self.reduce = ConvBnRelu(512, 64, 3, 1, 1,
                              has_bn=True, norm_layer=norm_layer,
                              has_relu=True, has_bias=False)
        self.ffm_refine = ConvBnRelu(576, 256, 1, 1, 0,
                              has_bn=True, norm_layer=norm_layer,
                              has_relu=True, has_bias=False)
        self.fpa = FPA(channels=2048,interp=True)
        self.arm = AttentionRefinement(2048, 512, norm_layer)
        self.refine = ConvBnRelu(512, 512, 3, 1, 1,
                              has_bn=True, norm_layer=norm_layer,
                              has_relu=True, has_bias=False)
        self.CA = CAM_Module(in_dim=512)
        self.PA = PAM_Module(in_dim=512)
        self.FFM = FeatureFusion(in_planes=1024, out_planes=512)
        self.output_head = BiSeNetHead(258, n_classes, 8, True, norm_layer)
    ...
    def forward(self, data):
        ...
        reduce = self.reduce(context_blocks[2])
        FPA = self.fpa(context_blocks[0])
        ARM = self.arm(FPA)
        refine = self.refine(ARM)
        ca = self.CA(refine)
        pa = self.PA(refine)
        ffm = self.FFM(ca, pa)
        fm = F.interpolate(ffm, size=context_blocks[2].shape[2:], mode="bilinear", align_corners=False)
        h = torch.cat((fm, reduce), dim=1)
        h = self.ffm_refine(h)
