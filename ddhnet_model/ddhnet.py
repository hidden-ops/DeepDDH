...
class DDHNet(nn.Module):
    def __init__(self, n_classes, n_channels, dropout1, dropout2, pretrained_model=None, norm_layer=nn.BatchNorm2d):
        ...
        self.CA = CAM_SE_Module(in_dim=512, dropout=dropout1)
        self.PA = PAM_Module(in_dim=512)
        #self.PA = PAM_SE_Module(in_dim=512, height=8, width=8, dropout=dropout2, cuda=True)
    ...
    def forward(self, data):
        ...
        x_range = torch.linspace(-1, 1, h.shape[-1], device=h.device)
        y_range = torch.linspace(-1, 1, h.shape[-2], device=h.device)
...
if __name__ == "__main__":
    model = DDHNet(n_classes=8, n_channels=3, dropout1=1.0, dropout2=1.0, pretrained_model=True)
