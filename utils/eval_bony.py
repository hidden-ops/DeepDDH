...
from utils.segLoss_raw import SegmentationLosses
from utils.HeatmapLoss import HeatmapLoss
...
def eval_net(net, loader, model_name, device):
    ...
    tot = 0
    mse = HeatmapLoss(loss_type="smooth_l1")

    with tqdm(total=n_val, desc='Validation round', unit='batch', leave=False) as pbar:
        for batch in loader:
            imgs, true_masks, true_bony = batch['image'], batch['mask'], batch['bony']
            imgs = imgs.to(device=device, dtype=torch.float32)
            true_masks = true_masks.to(device=device, dtype=torch.float32)
            true_bony = true_bony.to(device=device, dtype=torch.float32)
            ...
            if net.n_classes > 1:
                if model_name == 'bisenet':
                    ...
                    seg_loss = SegmentationLosses(cuda=True).CrossEntropyLoss(mask_pred, true_masks).item()
                    bony_loss = SegmentationLosses(cuda=True).CrossEntropyLoss(bony_pred, true_bony).item()
                    tot += (seg_loss + bony_loss)
            else:
                if model_name == 'bisenet':
